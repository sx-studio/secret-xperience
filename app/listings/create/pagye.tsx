...
export default function CreateListingPage() {
  ...
  const [form, setForm] = useState({ title: '', description: '', category: 'escorts', subcategory: '', price_from: '', price_to: '', city: '', country: 'Belgium' })
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.title || !form.city) { setError('Please fill in all required fields.'); return }
    ...
    const { error: err } = await supabase.from('listings').insert({
      ...
      city: form.city, country: form.country, active: true,
    })
    if (err) { setError(err.message); setLoading(false); return }
    setSuccess(true); setLoading(false)
  }

  const s: React.CSSProperties = { width: '100%', padding: '11px 14px', background: '#222', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: '8px', color: '#ece8e1', fontSize: '13px', outline: 'none', marginBottom: '1rem', fontFamily: 'sans-serif' }
  ...
  if (success) return (
    <div style={{ minHeight: '100vh', background: '#080808', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif' }}>
      <div style={{ textAlign: 'center', color: '#ece8e1' }}>
        ...
        <button onClick={() => window.location.href = '/'} style={{ padding: '0.75rem 1.5rem', background: '#c5a05a', border: 'none', borderRadius: '8px', color: '#080808', cursor: 'pointer', fontWeight: 600 }}>View platform</button>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080808', fontFamily: 'sans-serif', color: '#ece8e1' }}>
      <nav style={{ background: '#101010', borderBottom: '0.5px solid rgba(255,255,255,0.07)', padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={() => window.location.href = '/dashboard'} style={{ background: 'transparent', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: '8px', color: '#8c8880', padding: '0.5rem 1rem', cursor: 'pointer', fontSize: '13px' }}>Dashboard</button>
      </nav>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div>
              ...
              <input type="number" value={form.price_from} onChange={e => set('price_from', e.target.value)} placeholder="200" style={s} />
            </div>
            <div>
              ...
              <input type="number" value={form.price_to} onChange={e => set('price_to', e.target.value)} placeholder="1200" style={s} />
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div>
              ...
              <input value={form.city} onChange={e => set('city', e.target.value)} placeholder="Brussels" style={s} />
            </div>
            <div>
              ...
              <select value={form.country} onChange={e => set('country', e.target.value)} style={s}>
                ...
                <option>Germany</option><option>Luxembourg</option><option>United Kingdom</option><option>Other</option>
              </select>
            </div>
          </div>

          {error && <p style={{ color: '#b84d72', fontSize: '13px', marginBottom: '1rem' }}>{error}</p>}
          <button type="submit" disabled={loading} style={{ width: '100%', padding: '13px', background: 'linear-gradient(135deg,#c5a05a,#9a7a3a)', border: 'none', borderRadius: '8px', color: '#080808', cursor: 'pointer', fontWeight: 600, fontSize: '14px' }}>
