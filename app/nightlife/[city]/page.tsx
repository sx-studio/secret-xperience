...
export async function generateMetadata({ params }: { params: { city: string } }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://www.secretxperience.eu/nightlife/${cityKey}` },
  }
}

export default async function NightlifeCityPage({ params }: { params: { city: string } }) {
  const cityKey = params.city.toLowerCase()
  const cityName = CITIES[cityKey]
  ...
  const faq = cityFaq(cityName, 'nightlife venue')

  const itemList = {
    '@type': 'ItemList',
    name: `Nightlife in ${cityName}`,
    url: `https://www.secretxperience.eu/nightlife/${cityKey}`,
    ...
    <>
      ...
      <div style={{ background: S.bg, minHeight: '100vh', color: S.t, fontFamily: S.sans }}>
        ...
        <header style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(5,5,5,0.95)', backdropFilter: 'blur(12px)', borderBottom: `0.5px solid rgba(197,160,90,0.12)`, padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <a href="/nightlife" style={{ fontSize: 13, color: S.t2 }}>← All nightlife</a>
        </header>
        ...
        <main style={{ maxWidth: 1200, margin: '0 auto', padding: '3rem 24px 6rem' }}>
          <nav style={{ fontSize: 12, color: S.t2, marginBottom: '1.5rem', display: 'flex', gap: '6px', alignItems: 'center' }}>
            ...
            <a href="/nightlife" style={{ color: S.t2 }}>Nightlife</a><span>›</span>
          ...
          <h1 style={{ fontFamily: S.serif, fontSize: 'clamp(32px,5vw,52px)', fontWeight: 400, marginBottom: '0.5rem', lineHeight: 1.15 }}>
            Nightlife in <em style={{ fontStyle: 'italic', color: S.gold }}>{cityName}</em>
          </h1>
          <p style={{ fontSize: 14, color: S.t2, marginBottom: '1.25rem', maxWidth: 560, lineHeight: 1.7 }}>
            {listings?.length ?? 0} adult nightlife venue{listings?.length !== 1 ? 's' : ''} in {cityName}. Clubs, fetish bars, swingers nights and private events — all verified listings.
          </p>
          ...
          {(!listings || listings.length === 0) ? (
            <div style={{ textAlign: 'center', padding: '4rem 0', color: S.t2 }}>
              <p style={{ fontSize: 16, marginBottom: 16 }}>No nightlife listings yet in {cityName}.</p>
              <a href="/nightlife" style={{ color: S.gold, fontSize: 14 }}>Browse all nightlife →</a>
            </div>
          ) : (
                ...
                <Link key={l.id} href={`/listings/${l.id}`}>
                  <div className="city-card">
                    ...
                    <div className="city-card-body">
                      ...
                      <div className="city-card-meta"><span>📍 {l.city}</span></div>
                    </div>
                  </div>
                </Link>
              ))}
          ...
          {faq.length > 0 && (
            <section style={{ marginTop: '4rem', borderTop: `0.5px solid ${S.b}`, paddingTop: '2.5rem', maxWidth: 760 }}>
              <h2 style={{ fontFamily: S.serif, fontSize: 'clamp(24px,4vw,34px)', fontWeight: 400, marginBottom: '1.75rem' }}>
                Nightlife in {cityName} — <em style={{ color: S.gold }}>your questions</em>
              </h2>
          ...
          <div style={{ marginTop: '4rem', borderTop: `0.5px solid ${S.b}`, paddingTop: '2rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {Object.entries(CITIES).filter(([k]) => k !== cityKey).map(([k, name]) => (
                <a key={k} href={`/nightlife/${k}`} style={{ fontSize: 13, padding: '5px 14px', border: `0.5px solid ${S.b}`, borderRadius: '20px', color: S.t2, transition: 'border-color .2s,color .2s' }}
