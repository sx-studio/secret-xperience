...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticRoutes = ['/', '/events', '/advertise', '/search', '/discover', '/nightlife', '/terms', '/privacy', '/login', '/partners', '/regulations', '/medical', '/refer', '/creators', '/shop', '/how-it-works'].map(path => ({ url: BASE + path, lastModified: new Date(), changeFrequency: path === '/' ? 'daily' as const : 'weekly' as const, priority: path === '/' ? 1 : ['/search', '/discover', '/nightlife', '/events', '/advertise', '/regulations', '/medical'].includes(path) ? 0.8 : 0.7 }))
