...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticRoutes = ['/', '/events', '/advertise', '/login'].map(path => ({ url: BASE + path, lastModified: new Date(), changeFrequency: 'daily' as const, priority: path === '/' ? 1 : 0.7 }))
  const categoryRoutes = ['escorts', 'companionship', 'nightlife', 'creators', 'rentals', 'domination', 'massage', 'experiences'].map(cat => ({ url: `${BASE}/${cat}`, lastModified: new Date(), changeFrequency: 'daily' as const, priority: 0.9 }))
