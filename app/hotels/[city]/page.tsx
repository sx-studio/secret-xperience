...
export async function generateMetadata({ params }: { params: { city: string } }): Promise<Metadata> {
  ...
  return {
    title: `Adult-Friendly Hotels in ${cityName} | SecretXperience`,
    description: `Discreet, adult-friendly hotels and short stays in ${cityName}. Book securely on SecretXperience — no questions asked, verified listings.`,
    openGraph: {
      title: `Adult-Friendly Hotels in ${cityName} | SecretXperience`,
      description: `Find discreet hotels and short stay accommodations in ${cityName}. Verified, bookable directly.`,
      type: 'website',
    },
    alternates: { canonical: `https://www.secretxperience.eu/hotels/${cityKey}` },
  }
}

export default async function HotelsCityPage({ params }: { params: { city: string } }) {
  const cityKey = params.city.toLowerCase()
  const cityName = CITIES[cityKey]
    ...
    <>
      ...
      <div style={{ background: S.bg, minHeight: '100vh', color: S.t, fontFamily: S.sans }}>
        ...
        <header style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(5,5,5,0.95)', backdropFilter: 'blur(12px)', borderBottom: `0.5px solid rgba(197,160,90,0.12)`, padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <a href="/hotels" style={{ fontSize: 13, color: S.t2 }}>← All hotels</a>
        </header>
        ...
        <main style={{ maxWidth: 1200, margin: '0 auto', padding: '3rem 24px 6rem' }}>
          <nav style={{ fontSize: 12, color: S.t2, marginBottom: '1.5rem', display: 'flex', gap: '6px', alignItems: 'center' }}>
            ...
            <a href="/hotels" style={{ color: S.t2 }}>Hotels</a><span>›</span>
          ...
          {(!listings || listings.length === 0) ? (
            <div style={{ textAlign: 'center', padding: '4rem 0', color: S.t2 }}>
              <p style={{ fontSize: 16, marginBottom: 16 }}>No hotel listings yet in {cityName}.</p>
              <a href="/hotels" style={{ color: S.gold, fontSize: 14 }}>Browse all hotels →</a>
            </div>
          ) : (
          ...
          <div style={{ marginTop: '4rem', borderTop: `0.5px solid ${S.b}`, paddingTop: '2rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {Object.entries(CITIES).filter(([k]) => k !== cityKey).map(([k, name]) => (
                <a key={k} href={`/hotels/${k}`} style={{ fontSize: 13, padding: '5px 14px', border: `0.5px solid ${S.b}`, borderRadius: '20px', color: S.t2, transition: 'border-color .2s,color .2s' }}
