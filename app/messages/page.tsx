...
export default function MessagesPage() {
    ...
    async function init() {
      ...
      if (otherIds.length > 0) {
        ...
        profiles?.forEach(p => {
          ...
          if (c) c.other_name = p.full_name || p.username || p.id.slice(0, 8)
        })
      }
      ...
      if (providerId) {
        ...
        if (!conv) {
          const { data: profile } = await supabase.from('profiles').select('full_name, username').eq('id', providerId).single()
          conv = {
            other_id: providerId,
            other_name: profile?.full_name || profile?.username || 'Provider',
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `receiver_id=eq.${uid}` }, (payload) => {
          ...
          setConversations(prev => {
            ...
            if (!updated.find(c => c.other_id === otherId)) {
              updated.unshift({ other_id: otherId, other_name: otherId.slice(0, 8), listing_id: newMsg.listing_id, listing_title: null, last_message: newMsg.body, last_at: newMsg.created_at, unread: 1 })
            }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050505', color: '#ece8e1', fontFamily: "'Jost', sans-serif", display: 'flex', flexDirection: 'column' }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@300;400&family=Jost:wght@300;400;500&display=swap');*{box-sizing:border-box}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:rgba(197,160,90,0.15);border-radius:4px}`}</style>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        <div style={{ width: '280px', borderRight: '0.5px solid rgba(255,255,255,0.06)', overflowY: 'auto', flexShrink: 0 }}>
          {conversations.length === 0 && <div style={{ padding: '2rem 1.5rem', color: '#4c4a47', fontSize: '13px' }}>No conversations yet.</div>}
          {conversations.map(c => (
            <div key={c.other_id} onClick={() => { setActiveConv(c); const supabase = createClient(); supabase.from('messages').select('*').or(`sender_id.eq.${userId},receiver_id.eq.${userId}`).order('created_at', { ascending: true }).then(({ data }) => { setMessages((data || []).filter((m: Message) => (m.sender_id === userId && m.receiver_id === c.other_id) || (m.sender_id === c.other_id && m.receiver_id === userId))) }); setConversations(prev => prev.map(x => x.other_id === c.other_id ? { ...x, unread: 0 } : x)) }}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {!activeConv ? (
            ...
            <>
              <div style={{ padding: '1rem 1.5rem', borderBottom: '0.5px solid rgba(255,255,255,0.06)', background: '#080808', flexShrink: 0 }}>
              ...
              <div style={{ padding: '1rem 1.5rem', borderTop: '0.5px solid rgba(255,255,255,0.06)', background: '#080808', display: 'flex', gap: '10px', alignItems: 'flex-end', flexShrink: 0 }}>
                <textarea value={body} onChange={e => setBody(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage() } }} placeholder="Type a message…" rows={1} style={{ flex: 1, background: '#111', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '10px', padding: '10px 14px', color: '#ece8e1', fontSize: '13px', resize: 'none', fontFamily: 'inherit', lineHeight: 1.5, outline: 'none' }} />
