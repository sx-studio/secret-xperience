...
function CreatorCard({ l, isFeatured, idx }: { l: any; isFeatured: boolean; idx: number }) {
  ...
  const stars = Math.min(5, Math.round(l.rating || 4.5))
  ...
  return (
    <Link
      ...
      style={{
        ...
        background: 'var(--bg1)',
        border: isFeatured
          ? '1px solid rgba(197,160,90,0.5)'
          : '0.5px solid var(--b)',
        borderRadius: '16px',
        overflow: 'hidden',
        transition: 'transform 0.22s ease, box-shadow 0.22s ease, border-color 0.15s',
        boxShadow: isFeatured
          ? '0 0 24px rgba(197,160,90,0.15), 0 8px 32px rgba(0,0,0,0.4)'
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: 'linear-gradient(0deg, rgba(8,6,18,0.85) 0%, rgba(8,6,18,0.2) 50%, transparent 100%)',
        }} />
        {/* Monogram */}
        <div style={{
        ...
        {/* Top badges */}
        <div style={{ position: 'absolute', top: '10px', left: '10px', display: 'flex', gap: '5px', flexWrap: 'wrap', zIndex: 2 }}>
          {isFeatured && (
            <span style={{ fontSize: '10px', fontWeight: 700, padding: '3px 8px', borderRadius: '10px', background: 'rgba(197,160,90,0.2)', color: 'var(--gold)', border: '0.5px solid rgba(197,160,90,0.4)', backdropFilter: 'blur(4px)' }}>
              ✦ Featured
            </span>
          )}
          {l.verified && (
            <span style={{ fontSize: '10px', fontWeight: 700, padding: '3px 8px', borderRadius: '10px', background: 'rgba(38,212,160,0.15)', color: '#26d4a0', border: '0.5px solid rgba(38,212,160,0.3)', backdropFilter: 'blur(4px)' }}>
              ✓ Verified
            </span>
          )}
          {l.premium && (
            <span style={{ fontSize: '10px', fontWeight: 700, padding: '3px 8px', borderRadius: '10px', background: 'rgba(139,92,246,0.15)', color: '#a78bfa', border: '0.5px solid rgba(139,92,246,0.3)', backdropFilter: 'blur(4px)' }}>
              Premium
            </span>
          )}
        </div>
      ...
      <div style={{ padding: '1rem 1.1rem 1.1rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '13px', marginBottom: '8px' }}>
          <span style={{ color: 'var(--gold)' }}>
            ...
            {l.rating > 0 && <span style={{ color: 'var(--t3)', marginLeft: '4px' }}>{Number(l.rating).toFixed(1)}</span>}
          </span>
          {l.city && (
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        .creator-card:hover { transform: translateY(-4px); box-shadow: 0 16px 48px rgba(0,0,0,0.55) !important; }
        .creator-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 18px; }
        .filter-pill { height: 34px; padding: 0 16px; border-radius: 20px; border: 0.5px solid var(--b); background: transparent; color: var(--t2); font-size: 13px; white-space: nowrap; display: inline-flex; align-items: center; text-decoration: none; transition: background 0.15s, color 0.15s, border-color 0.15s; cursor: pointer; }
        .filter-pill:hover { background: var(--bg2); color: var(--t); border-color: var(--b2); }
        .filter-pill.active { background: rgba(197,160,90,0.12); color: var(--gold); border-color: rgba(197,160,90,0.35); }
      ...
      <div style={{ background: 'var(--bg)', minHeight: '100vh', color: 'var(--t)', fontFamily: 'var(--sans)' }}>
        ...
        <div style={{
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', position: 'relative' }}>
            ...
            <p style={{ fontSize: '15px', color: 'var(--t2)', maxWidth: '520px', lineHeight: 1.75, marginBottom: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <span style={{ width: '1px', height: '14px', background: 'var(--b3)' }} />
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2.5rem 1.5rem 6rem' }}>
          ...
          {featured.length > 0 && (
            <div style={{ marginBottom: '3rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1.25rem' }}>
                ...
                <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.15em', color: 'var(--gold)', textTransform: 'uppercase' }}>
                  ✦ Featured Creators
                </div>
          ...
          {!listings || listings.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '5rem 2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem' }}>
              ...
              <p style={{ fontSize: '14px', color: 'var(--t3)', maxWidth: '340px', lineHeight: 1.7 }}>No creators yet in this space. Be among the first and build a following from day one.</p>
          ...
          {allListings.length > 0 && (
            <div style={{
              ...
              <div style={{ position: 'relative', zIndex: 1 }}>
                <div style={{ fontFamily: 'var(--serif)', fontSize: '24px', marginBottom: '0.75rem' }}>
                  Are you a creator?
                </div>
